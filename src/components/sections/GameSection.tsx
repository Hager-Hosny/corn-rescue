...
const GameSection = () => {
  ...
  const [highScore, setHighScore] = useState(() =>
    parseInt(localStorage.getItem("corn-defender-high") || "0", 10)
  );
        ...
        <div className="max-w-lg mx-auto">
          {gameState === "menu" && (
            <div className="flex flex-col items-center gap-6 bounce-in">
              ...
              {highScore > 0 && (
                <p className="text-secondary font-display font-bold text-lg">🏆 Best: {highScore}</p>
              )}
              <button
                onClick={startGame}
          ...
          {gameState === "playing" && (
            <div>
              <div className="flex justify-between items-center mb-4 px-2">
                <div className="font-display font-bold text-lg text-foreground">⭐ {score}</div>
              ...
              <div className="text-center mt-3">
                <span className="text-xs text-muted-foreground font-body">Level {difficulty}</span>
              </div>
            </div>
          )}
          ...
          {gameState === "gameover" && (
            <div className="flex flex-col items-center gap-5 bounce-in">
              ...
              <h3 className="font-display text-3xl font-bold text-accent">Game Over!</h3>
